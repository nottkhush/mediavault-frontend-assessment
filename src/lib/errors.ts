...
export function describeError(
  ...
  if (error instanceof ApiError) {
    if (error.code === 'legal_hold') return 'This asset is on legal hold, so it cannot be archived.';
    if (error.code === 'write_failed') return "We couldn't save that change. Please try again.";
    if (error.status === 0) {
      return isOffline()
